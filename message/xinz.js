...
\`\`\`▢ Envio : ${yut.videos[0].ago}\`\`\`
\`\`\`▢ Assistiu : ${yut.videos[0].views}\`\`\`
...
\`\`\`▢ Duração : ${yut.videos[0].timestamp}\`\`\`
\`\`\`▢ Link : ${a.data}\`\`\`
_Para a duração de mais do que o limite é apresentado na forma de um link_`, msg)
...
//------------------< G R U P >-------------------
            ...
            case prefix+'groupinfo':
                ...
                let ingfo = `*G R O U P I N F O*\n\n*Nome :* ${groupName}\n*ID Grupo :* ${from}\n*Feito :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Grupo de Proprietários :* @${groupMetadata.owner.split('@')[0]}\n*Número de Administradores :* ${groupAdmins.length}\n*Número de participantes :* ${groupMembers.length}\n*Bem-vindo :* ${isWelcome ? 'Aktif' : 'Mati'}\n*Left :* ${isLeft ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*AntiBadword :* ${isBadword ? 'Aktif' : 'Mati'}\n*Desc :* \n${groupMetadata.desc}`
...
//------------------< Enable / Disable >-------------------
            ...
            case prefix+'antilink':
                ...
                if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
                if (!isBotGroupAdmins) return reply(mess.BotAdmin)
                if (args.length === 1) return reply(`Selecione habilitar ou desabilitar`)
                if (args[1].toLowerCase() === 'enable'){
                    if (isAntiLink) return reply(`Já ativo`)
                    antilink.push(from)
					fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
					reply('Grupo anti-link ativo')
                } else if (args[1].toLowerCase() === 'disable'){
                    let anu = antilink.indexOf(from)
                    antilink.splice(anu, 1)
                    fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
                    reply('Antilink de grupo está desativado')
                } else {
